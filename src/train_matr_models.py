"""Fit immutable, lineage-safe XGBoost RUL candidate generations."""
import argparse
...
XGBOOST_CONFIG = {"n_estimators": 200, "max_depth": 6, "learning_rate": 0.05, "n_jobs": 4, "random_state": 42}
...
def generation_plan(manifest, lifecycle_events, published_counts=None, *, output_root=None):
    ...
    fingerprint = model_fingerprint(battery_ids, manifest_fingerprint=manifest_fingerprint, split_version=SPLIT_VERSION, feature_version=FEATURE_VERSION, model_config={"xgboost": XGBOOST_CONFIG, "feature_list_hash": feature_hash})
    ...
    metadata = {"generation": generation, "model_fingerprint": fingerprint, "training_battery_count": threshold, "snapshot_lineage_checksum": sha256("\n".join(battery_ids).encode()).hexdigest(), "arrival_manifest_fingerprint": manifest_fingerprint, "split_version": SPLIT_VERSION, "feature_version": FEATURE_VERSION, "feature_list_hash": feature_hash, "xgboost_config": XGBOOST_CONFIG}
    return {"battery_ids": battery_ids, "training_battery_count": threshold, "fingerprint": fingerprint, "generation": generation, "model_version": f"matr-rul-xgboost-{generation}-{fingerprint[:12]}", "metadata": metadata, "artifact_dir": artifact_dir}
...
def train_generation(plan, *, root=ROOT, evaluated_at=None):
    ...
    model = make_pipeline(SimpleImputer(strategy="median"), XGBRegressor(**XGBOOST_CONFIG))
    model.fit(matrix[indexes["train"]], labels[indexes["train"]])
    raw_predictions = model.predict(matrix)
    score = {split: metrics(labels[index], np.maximum(model.predict(matrix[index]), 0)) for split, index in indexes.items()}
    ...
    test_stages, test_predictions = stages[indexes["test"]], np.maximum(model.predict(matrix[indexes["test"]]), 0)
    ...
    metadata = {**plan["metadata"], **{f"{split}_row_count": int(index.sum()) for split, index in indexes.items()}}
    evaluation = {"model_version": plan["model_version"], "model_name": "xgboost_rul_regressor", "dataset": "MATR", "status": "candidate", "evaluated_at": evaluated_at, "metrics_json": json.dumps(score, sort_keys=True), "training_metadata_json": json.dumps(metadata, sort_keys=True)}
