...
from src.shared_features import (
    ...
    generation_id_for,
    load_current_monitoring_feature_rows,
...
from src.feature_contract import RUL_FEATURES
from src.stream_state import build_compact_finalized_cycle_boundary, build_finalized_cycle_boundary
...
def test_current_monitoring_features_include_non_training_batteries_from_full_compatible_source(tmp_path):
    root = tmp_path / "matr"
    source = root / "degradation_features"
    source.mkdir(parents=True)
    def feature(battery, cycle):
        return {"dataset": "MATR", "battery_id": battery, **{name: cycle if name == "cycle_index" else 1.0 for name in RUL_FEATURES}}
    pq.write_table(pa.Table.from_pylist([
        feature("train", 1), feature("train", 2), feature("validation", 6), feature("test", 8),
    ]), source / "part.parquet")
    contract = "degradation-features:" + sha256(",".join(RUL_FEATURES).encode()).hexdigest()

    rows = load_current_monitoring_feature_rows(root, {"feature_contract_version": contract})

    assert [(row["battery_id"], row["cycle_index"]) for row in rows] == [
        ("test", 8), ("train", 2), ("validation", 6),
    ]
    assert [row["replay_sequence"] for row in rows] == [0, 0, 0]
